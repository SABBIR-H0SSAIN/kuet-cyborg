            ...
            <main class="main-content">
                ...
                <div class="settings-card glass">
                    <div class="page-header" style="margin-bottom:1rem; border-bottom: 1px solid var(--border-color); padding-bottom: 0.5rem;">
                        <h2 class="settings-section-title" style="border-bottom:none; margin-bottom:0; padding-bottom:0;">Battle Statisticts</h2>
                    </div>

                    <div class="grid-container">
                        <asp:GridView ID="gvStats" runat="server" AutoGenerateColumns="false" 
                            CssClass="styled-table" GridLines="None" 
                            OnRowCommand="gvStats_RowCommand" DataKeyNames="id">
                            <Columns>

                                <asp:BoundField DataField="target" HeaderText="Target Number" ItemStyle-Width="150px" />
                                <asp:BoundField DataField="label" HeaderText="Label / Achievement" />
                                <asp:TemplateField HeaderText="Actions" ItemStyle-Width="100px">
                                    <ItemTemplate>
                                        <asp:LinkButton ID="btnEdit" runat="server" 
                                            CommandName="EditStat" CommandArgument='<%# Eval("id") %>' 
                                            CssClass="action-btn btn-success" ToolTip="Edit Stat" CausesValidation="false">
                            ...
                            <EmptyDataTemplate>
                                <div style="padding: 2rem; text-align: center; color: var(--text-muted);">
                                    No battle statistics found in the database.
                                </div>
                            </EmptyDataTemplate>
                        </asp:GridView>
                    </div>
                ...
                <asp:Panel ID="pnlModal" runat="server" CssClass="modal-bg" Visible="false">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h3><asp:Literal ID="litModalTitle" runat="server" Text="Add Battle Stat"></asp:Literal></h3>
                            <asp:LinkButton ID="btnCloseModal" runat="server" OnClick="btnCloseModal_Click" style="color:var(--text-muted); text-decoration:none;" CausesValidation="false">✕</asp:LinkButton>
                        </div>
                        <asp:HiddenField ID="hfStatId" runat="server" />

                        <div class="form-group">
                            <label for="txtTarget">Target Number (e.g. 150)</label>
                            <asp:TextBox ID="txtTarget" runat="server" CssClass="form-control" required="required" type="number" min="0" placeholder="e.g. 150"></asp:TextBox>
                        </div>
                        <div class="form-group">
                            <label for="txtLabel">Label / Description (e.g. Matches Played)</label>
                            <asp:TextBox ID="txtLabel" runat="server" CssClass="form-control" required="required" placeholder="e.g. Matches Played"></asp:TextBox>
                        </div>
                        <div style="margin-top: 1.5rem; text-align: right;">
                            <asp:Button ID="btnSaveStat" runat="server" Text="Save Stat" CssClass="btn btn-primary" style="width: auto; padding: 0.75rem 2rem;" OnClick="btnSaveStat_Click" />
                        </div>
                    </div>
                </asp:Panel>
            </main>
